...
function Header({ displayLogo, title}) {
  ...
  return (
    <header>
      <div onClick={handleLogoChange} className={`logo ${displayLogo ? 'center' : ''}`}>
          {displayLogo ? (
          <>
      ...
      <nav>
        <ul>
          ...
          {language === 'no' ? (
            <Button onClick={()=> changeLanguage("en")} value="language.en"  ></Button>
          ) : (
            <Button onClick={() => changeLanguage('no')} value="language.no"  ></Button>
          )}
